...
The stochastic nature of these transitions means that even under identical initial conditions, the trajectory of an epidemic can vary due to random fluctuations. This randomness is particularly significant in small populations or during the early stages of an outbreak, where stochastic effects can influence the probability of disease extinction or large-scale spread. As the population size increases, stochastic fluctuations average out, and the system's behavior becomes increasingly deterministic. This transition, known as the mean-field limit, is achieved by considering the expected rates of change in the compartments rather than the random transitions of individuals. In this framework, the stochastic processes governing transitions are replaced by deterministic flows based on the average interaction rates.\cite{armbruster2017elementary}\\
