...
\begin{figure}[ht]
    ...
    % Inflows
    \node[left=2cm of sum] (inflow1) {$\mu I_{2}$};
    \node[above=0.5cm of inflow1] (inflow2) {$\mu I_{1}$};
    \node[below=0.5cm of inflow1] (inflow3) {$\mu I_{3}$};

    % Arrows from inflows to sum node
    \draw[->] (inflow1) -- (sum);
    \draw[->] (inflow2) -- (sum);
    \draw[->] (inflow3) -- (sum);

    % Arrow from sum node to rho
    \draw[->] (sum) -- (rho);

    % Arrow from rho to subpopulation, label midpoint
    \draw[->] (rho) -- node[pos=0.5, above] (midpoint) {$M_{(I);x}$} (subpop);



    % Multiplication by $\lambda$ node
    \node[draw, circle, below=1cm of midpoint] (lambda_mult) {$\lambda$};

    % Arrow from midpoint to lambda_mult with label
    ...
    % Arrow from lambda_mult to integrator
    \draw[->] (lambda_mult) -- node[pos=0.5, below] {$\dot{\rho}$} (integrator);

    % Arrow from integrator back to rho
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[node distance=2cm, >=Stealth]
        ...
        \node[draw, circle, above=0.5cm of midpoint, xshift=-1.5cm] (mu) {$\mu$};
        \node[draw, circle, below=1cm of midpoint] (lambda_mult) {$ \lambda$};
        \draw[->] (midpoint) -- (lambda_mult);
        \node[draw, rectangle, left=1cm of lambda_mult, minimum width=1cm, minimum height=1cm] (integrator) {$\displaystyle 1-\int$};
        ...
        \node[draw, circle, above=0.5cm of midpoint_mirror, xshift=1.5cm] (mu_mirror) {$\mu$};
